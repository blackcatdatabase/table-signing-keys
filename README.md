...
<div style='font-size:13px;opacity:0.92;'>Lineage heat: 3 outbound / 2 inbound · ✅ No engine drift detected · Index coverage: ready · PII: 7 · Changelog: fresh (18 d)</div>
</div>
...
- Pkg folder: [packages\signing-keys](https://github.com/blackcatacademy/blackcat-database/blob/main/packages\signing-keys)
...
graph LR
  ...
  signing_keys["signing_keys"]:::center
  signing_keys -->|FK| crypto_algorithms["crypto_algorithms"]:::link
  signing_keys -->|FK| kms_keys["kms_keys"]:::accent
  signing_keys -->|FK| users["users"]:::link
  rbac_repositories["rbac_repositories"]:::inbound -->|FK| signing_keys
  signatures["signatures"]:::inbound -->|FK| signing_keys
...
| Inbound links (tables depending on this) | 2 |
| Views | 4 |
| Seeds | 0 |
| Drift warnings | 0 |
| PII flags | 7 |
...
- Outbound (depends on): "crypto_algorithms", "kms_keys", "users"
- Inbound (relies on this): "rbac_repositories", "signatures"
...
## Usage
- Install/upgrade schema: pwsh -NoLogo -NoProfile -File scripts/schema-tools/Migrate-DryRun.ps1 -Package signing-keys -Apply
