...
namespace BlackCat\Database\Packages\SigningKeys\Repository;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\SigningKeys\Definitions;
use BlackCat\Database\Packages\SigningKeys\Criteria;
use BlackCat\Database\Packages\SigningKeys\Dto\SigningKeyDto as Dto;
use BlackCat\Database\Packages\SigningKeys\Mapper\SigningKeyDtoMapper as RowMapper;
...
final class SigningKeyRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\SigningKeys\Definitions::class; }
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [ 'name' ] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\SigningKeys\Definitions::table());
            return $bulk->upsertMany($rows, [ 'name' ], [ 'algo_id', 'public_key', 'private_key_enc', 'kms_key_id', 'origin', 'status', 'scope', 'activated_at', 'retired_at', 'notes' ]);
        }
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\SigningKeys\Dto\SigningKeyDto|null */
    public function getByName(string $name, bool $asDto = false): array|\BlackCat\Database\Packages\SigningKeys\Dto\SigningKeyDto|null {
        $row = $this->getByUnique([ 'name' => $name ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\SigningKeys\Mapper\SigningKeyDtoMapper::fromRow($row);
    }
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\SigningKeys\Dto\SigningKeyDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\SigningKeys\Dto\SigningKeyDto|null {
