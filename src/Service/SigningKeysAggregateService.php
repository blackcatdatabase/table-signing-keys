...
namespace BlackCat\Database\Packages\SigningKeys\Service;

use BlackCat\Core\Database;
use BlackCat\Database\Packages\SigningKeys\Dto\SigningKeyDto;
use BlackCat\Database\Packages\SigningKeys\Mapper\SigningKeyDtoMapper;
 ...
 */
final class SigningKeysAggregateService
{
    use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private SigningKeyRepository $signingKeyRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'signing_keys' => $this->signingKeyRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if (!$row) {
                ...
                throw new \BlackCat\Database\Packages\SigningKeys\ModuleException("Record {$idStr} not found for locking.");
            }
